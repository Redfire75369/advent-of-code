...
function part1() {
	...
	for (let i = 0; i < inputs.length; i++) {
		...
		for (let j = 0; j < requiredFields.length; j++) {
			valid &= input.includes(requiredFields[j]);
		}
				...
				if (key === Object.keys(requiredFields)[j]) {
					if (key.substring(1) === "yr") {
						if (field.split(":")[1].match(requiredFields[key])) {
							let year = Number(field.split(":")[1].match(/^\d{4}/)[0]);
							switch (key) {
								case "byr":
									validField |= year <= 2002 && year >= 1920;
									break;
								case "iyr":
									validField |= year <= 2020 && year >= 2010;
									break;
								case "eyr":
									validField |= year <= 2030 && year >= 2020;
									break;
							}
						}
					}else if (key === "hgt") {
						if (field.split(":")[1].match(/^\d{3}cm/)) {
							...
							validField |= Number(field.split(":")[1].match(/^\d{2}/)[0]) <= 76 && Number(field.split(":")[1].match(/^\d{2}/)[0]) >= 59;
						}
					} else {
						validField |= field.split(":")[1].match(requiredFields[key]) != null;
					}
				}
			}
			if (!validField) {
